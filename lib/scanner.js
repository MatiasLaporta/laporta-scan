...
function buildCwv(psi) {
  ...
  // INP (solo campo) — si no hay datos de campo, no hay equivalente lab: sin rating ni punto.
  if (f.inp) push('INP', 'Interaction to Next Paint', fmtMs(f.inp.value), catToBand(f.inp.category), 'campo');
  else push('INP', 'Interaction to Next Paint', 'Sin datos', '', 'n/d');
  // CLS — un CLS de 0 se muestra como "0", no "0.000"
  const fmtCls = (v, dec) => v === 0 ? '0' : v.toFixed(dec);
  if (f.cls) push('CLS', 'Cumulative Layout Shift', fmtCls(f.cls.value, 2), catToBand(f.cls.category), 'campo');
  else push('CLS', 'Cumulative Layout Shift', lab.cls != null ? fmtCls(lab.cls, 3) : '—', metricBand('cls', lab.cls), 'lab');
  // FCP
...
function analyzeContent(url, seo) {
  ...
  // La frescura solo es un factor relevante en páginas de contenido/artículo.
  // Una landing/home (aunque sea larga) NO debe penalizarse por no tener fecha.
  // Señales de "contenido datable": schema Article, URL de blog/artículo o fecha ya declarada.
  const path = (() => { try { return new URL(url).pathname; } catch { return ''; } })();
  const blogUrl = /\/(blog|posts?|articles?|news|noticias?|art[ií]culos?)(\/|$)/i.test(path) || /\/20\d{2}\/\d{1,2}\//.test(path);
  const freshnessApplies = seo.schema.hasArticle || blogUrl || !!pubDate;
  ...
  return {
    ...
    freshness: { hasDate: !!pubDate, ageDays, ok: freshnessOk, applies: freshnessApplies },
...
function scoreAeo(content, files, seo) {
  const checks = [
    ...
    // Frescura: solo se incluye en páginas de contenido/artículo (ver freshness.applies).
    // En una landing sin blog no es un factor, así que ni se muestra ni puntúa.
    ...(content.freshness.applies ? [{ key: 'freshness', ok: content.freshness.ok, weight: 10,
  ...
  // Reescalado a 100 sobre los checks aplicables (si frescura no aplica, el máximo baja a 90 y se normaliza).
  const maxWeight = checks.reduce((s, c) => s + c.weight, 0);
  const earned = checks.reduce((s, c) => s + (c.ok ? c.weight : 0), 0);
  let score = maxWeight ? (earned / maxWeight) * 100 : 0;
  // Gate: bots de citación de IA bloqueados → penalización fuerte
  if (!files.robots.allowsAiSearch) score = score * 0.6;
...
function buildRecommendations(psi, seo, content, files, seoRes, aeoRes) {
  ...
  if (content.freshness.applies && !content.freshness.ok) add('medium', 'AEO/GEO', 'Sin fecha visible',
    'Agrega fecha de publicación/actualización (schema Article o <time>) en tus artículos. La frescura correlaciona con ser citado.');
...
function consolidate(psi, seo, content, files) {
  ...
  // Estado de PSI: 'ok' | 'missing-key' (no hay API key) | 'error' (timeout/fallo para esta URL)
  const psiStatus = !psi || psi.unavailable ? (psi && psi.error ? 'error' : 'missing-key') : 'ok';

  return {
    globalScore,
    band: band(globalScore),
    psiStatus,
    psiError: (psi && psi.error) || null,

...
function scoreSeo(seo, files) {
  const checks = [
    ...
    { key: 'social', ok: (seo.og.title && seo.og.image) || seo.twitter.card, weight: 2,
      label: 'Open Graph / Twitter', detail: seo.og.image ? 'og:image presente' : 'sin og:image' },
  ];
...
async function runScan(rawUrl) {
  ...
  return { url, timing: { ms: Date.now() - t0 }, psi, seo, content, files, consolidated };
}
...
module.exports = { PSI_API_KEY, normalizeUrl, runScan, saveLead };

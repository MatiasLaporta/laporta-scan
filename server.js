/* ============================================================
 * Laporta Scan · backend
 * Auditoría web: Performance (Google PageSpeed) + SEO técnico + AEO/GEO/LLMO
 *
 * Endpoints
 *   POST /api/scan  { url }           -> análisis completo (psi + seo + files + consolidated)
 *   POST /api/lead  { name, email, … } -> guarda el lead en data/leads.jsonl
 *
 * Scoring (replicado 1:1 del análisis original):
...
app.post('/api/scan', async (req, res) => {
  const parsed = normalizeUrl(req.body && req.body.url);
  if (!parsed) return res.status(400).json({ error: 'URL inválida. Ej: tudominio.cl' });
  ...
  try {
    ...
    try {
      ...
      return res.status(502).json({ error: 'No se pudo acceder al sitio. Verifica que la URL sea pública y accesible.' });
    }
    if (!page.body) return res.status(502).json({ error: 'El sitio no devolvió contenido HTML.' });
    ...
    res.status(500).json({ error: 'Error interno al analizar la URL.' });
  }
});
...
app.post('/api/lead', async (req, res) => {
  ...
  try {
    ...
    console.error('[lead] no se pudo escribir en disco:', e.message);
    return res.status(500).json({ error: 'No se pudo guardar el lead.' });
  }

  // Entrega opcional en tiempo real (Make / Zapier / n8n / endpoint propio)
  if (LEAD_WEBHOOK_URL) {
...
app.listen(PORT, () => {
  console.log(`▸ Laporta Scan escuchando en http://localhost:${PORT}`);
